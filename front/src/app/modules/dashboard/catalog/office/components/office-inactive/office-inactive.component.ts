...
@Component({
  selector: 'app-office-inactive',
  templateUrl: './office-inactive.component.html',
  styleUrls: ['./office-inactive.component.scss'],
  providers: [ConfirmationService, MessageService],
})
export class OfficeInactiveComponent {
  value2!: string;
  name = 'Oficinas';
  office!: Office[];
  messages!: Message[];
  ...
  async getOffice(): Promise<Office[]> {
    const result = await this.officeService.get_all_inactive_office();
    this.office = result;
      ...
      this.messages = [
        {
          severity: 'info',
          summary: '',
          detail: 'Actualmente no tienes ninguna oficina inactiva.',
        },
      ];
    }
    return result;
  ...
  async Active(office_uid: string): Promise<any> {
