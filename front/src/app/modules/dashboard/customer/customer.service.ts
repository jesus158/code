...
export class CustomerService {
  ...
  async post_customer(customer: Customer): Promise<any> {
      ...
      try {
        const result = await firstValueFrom(
          this.http.post<Customer>(
            `${environment.ApiUrl}/customer/${db_access}/${business_uid}`,
            customer
          )
        );
        if (result) {
      ...
      try {
        const result = await firstValueFrom(
          this.http.put<Customer>(
            `${environment.ApiUrl}/customer/${customer_uid}/${db_access}/${business_uid}`,
            customer
          )
        );
  ...
  async get_all_customer(): Promise<any> {
    const db_access = this.cookieService.get('a_t');
    const business_uid = this.cookieService.get('b_a');
    if (db_access && business_uid) {
      try {
  ...
  async get_all_inactive_customer(): Promise<any> {
    ...
    if (db_access && business_uid) {
      try {
        const data = await firstValueFrom(
          this.http.get<Customer>(
            `${environment.ApiUrl}/customer/inactive/${db_access}/${business_uid}`
          )
        );
        return data.response;
      } catch (e: any) {
  ...
  async get_all_active_customer(): Promise<any> {
    ...
    if (db_access && business_uid) {
      try {
        const data = await firstValueFrom(
          this.http.get<Customer>(
            `${environment.ApiUrl}/customer/active/${db_access}/${business_uid}`
          )
        );
        return data.response;
      } catch (e: any) {
  ...
  async get_Customer(customer_uid: string | null | undefined): Promise<any> {
    const db_access = this.cookieService.get('a_t');
    const business_uid = this.cookieService.get('b_a');
    if (db_access && business_uid) {
      try {
        const data = await firstValueFrom(
          this.http.get<Customer>(
            `${environment.ApiUrl}/customer/one/${db_access}/${customer_uid}/${business_uid}`
          )
        );
        return data.response;
      } catch (e: any) {
